...
Public Class Stats2
    ...
    Private Sub getList(ByVal UneDateDebut As String, ByVal UneDateFin As String)
        Try
            ...
            UneRequete = "select count(*) as NbManifeste,sum(sg.manifNbreArticle) [NbrBL],FORMAT( sg.DATETRANSMISSION,'dd-MM-yyyy') as'DateTransmission',datename(weekday,sg.DATETRANSMISSION) as 'Jour'
            ,DATEPART(month,DATETRANSMISSION) Mois,datepart(day,DATETRANSMISSION) [N° Jour],datepart(year,DATETRANSMISSION) Annee
            from V_SEGMENTGEN sg where 1=1" & UnCritere & " group by  FORMAT( sg.DATETRANSMISSION,'dd-MM-yyyy'),datename(weekday,sg.DATETRANSMISSION),DATEPART(month,DATETRANSMISSION),datepart(day,DATETRANSMISSION),datepart(year,DATETRANSMISSION)
            order by 7,5,6 desc"
